...
class MujocoPolicy(Policy):
    ...
    def rollout(self, env, *, render=False, timestep_limit=None, save_obs=False, random_stream=None, policy_seed=None, bc_choice=None):
        ...
        #BJ - the m value is the multiplier that will be applied to the radial constraint
        env.env.m = 1
        #BJ - the timeflipper value is the timestep at which the .25 cost multiplier will be applied
        timeflipper = 500
        for _ in range(timestep_limit):
            #BJ - here the flipper boolean is changed
            if(t > timeflipper):
                env.env.flipper = True
            else:
                env.env.flipper = False
            ac = self.act(ob[None], random_stream=random_stream)[0]
            if save_obs:
                obs.append(ob)
            ob, rew, done, _ = env.step(ac)
